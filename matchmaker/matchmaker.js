...
module.exports = async (ws) => {
    ...
    queue.push(player);
    log.connection(`Matchmaker: player joined queue (total: ${queue.length})`);
    ...
    ws.on('close', () => {
        ...
        log.connection(`Matchmaker: player left queue (total: ${queue.length})`);
        updateQueueConfigs();
    });
...
function SessionAssignment(ws, matchId) {
    log.connection(`Matchmaker: session assignment matchId=${matchId}`);
    ws.send(JSON.stringify({
        "payload": {
            "matchId": matchId,
...
function Join(ws, matchId, sessionId) {
    log.connection(`Matchmaker: sending join matchId=${matchId} sessionId=${sessionId}`);
    ws.send(JSON.stringify({
        "payload": {
            "matchId": matchId,
            "sessionId": sessionId,
