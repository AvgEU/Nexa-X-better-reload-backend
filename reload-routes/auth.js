...
app.post("/account/api/oauth/token", async (req, res) => {
    ...
    switch (req.body.grant_type) {
        ...
        case "password":
            ...
            // Auto-register any account on first login (like Stock Nexa - no @projectreboot.dev needed)
            rebootAccount = true;
            log.debug(`Auto-register login for: ${email}`);
            {
                const normalizedEmail = email.includes("@") ? email.toLowerCase() : `${email.toLowerCase()}@nexa.local`;
                ...
                if (findUser) {
                    ...
                    await functions.registerUser(numberWith8Digits, displayName, normalizedEmail, password, true);
                    req.user = await User.findOne({ email: normalizedEmail });
                }
                req.body.username = normalizedEmail;
            }

            if (!req.user) {
                log.debug("Failed to find or create user");
                return error.createError(
