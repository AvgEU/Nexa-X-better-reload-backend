...
function resolveXmppPort() {
    if (typeof config.xmppPort === "number") return config.xmppPort;
    if (typeof config.matchmakerIP === "string" && config.matchmakerIP.includes(":")) {
        const parsed = parseInt(config.matchmakerIP.split(":").pop(), 10);
        if (!Number.isNaN(parsed)) return parsed;
    }
    return config.bEnableHTTPS ? 443 : 8080;
}

const port = resolveXmppPort();
let wss;
...
if (config.bEnableHTTPS) {
    ...
    httpsServer.listen(port, () => {
        ...
        if (err.code === "EADDRINUSE") {
            log.error(`Port ${port} is already in use (XMPP/Matchmaker). Stop the other backend or change xmppPort / matchmakerIP in Config/config.json`);
            process.exit(1);
        }
        throw err;
    });
...
wss.on('connection', async (ws) => {
    ws.on('error', (err) => {
        log.connection(`WebSocket error: ${err?.message || err}`);
    });

    const protocol = (ws.protocol || "").toLowerCase();
    if (protocol != "xmpp") {
        log.connection(`Matchmaker WebSocket open (protocol=${protocol || "default"})`);
        return matchmaker(ws);
    }
    log.connection("XMPP WebSocket open");
        ...
        switch (msg.root.name) {
            ...
            case "iq":
                ...
                switch (msg.root.attributes.id) {
                    ...
                    case "_xmpp_session1":
                        ...
                        await updatePresenceForFriends(ws, "{}", false, false);
                    break;
            ...
            case "presence":
                ...
                if (msg.root.attributes.type === "unavailable") {
                    if (msg.root.attributes.to && (msg.root.attributes.to.endsWith(`@muc.${global.xmppDomain}`) || msg.root.attributes.to.split("/")[0].endsWith(`@muc.${global.xmppDomain}`))) {
